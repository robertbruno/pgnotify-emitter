...
const HEALTHCHECK_METRICS_MATCH_REGEX=process.env.HEALTHCHECK_METRICS_MATCH_REGEX || /pgnotify_emitter_tests_errors*\ (\d+)/g
        ...
        res.on("data", (chunk) => {
            ...
            try {
                if(Number.parseInt(HEALTHCHECK_ENABLED_METRICS) == 0) {
                    ...
                    try {
                        const messagesCount = found[0].split(" ")[1]
                        if(!messagesCount){
                            console.log("[HEALTHCHECK] Not messages")
                            process.exit(5);
                        }
                        console.log("[HEALTHCHECK] pgnotify_emitter_messages", messagesCount)
